...
(->
  ...
  parchment.className = "error"  unless Detect.supportsWebSocket()
  document.querySelector("body").className += " upscaled" unless ctx.mozImageSmoothingEnabled?
...
initApp = ->
  ...
  $ ->
    ...
    $("#notifications div").bind Util.TRANSITIONEND, app.resetMessagesPosition.bind(app)
    $(".close").click ->
      app.hideWindows()
...
initGame = ->
  Game = require("./game")
  canvas = document.getElementById("entities").firstChild
  background = document.getElementById("background").firstChild
  foreground = document.getElementById("foreground").firstChild
  ...
  $(document).keyup (e) ->
    ...
    if game.started and not $("#chatbox").hasClass("active")
      switch key
        when Types.Keys.LEFT, Types.Keys.A, Types.Keys.KEYPAD_4
          game.player.moveLeft = false
          game.player.disableKeyboardNpcTalk = false
        when Types.Keys.RIGHT, Types.Keys.D, Types.Keys.KEYPAD_6
          game.player.moveRight = false
          game.player.disableKeyboardNpcTalk = false
        when Types.Keys.UP, Types.Keys.W, Types.Keys.KEYPAD_8
          game.player.moveUp = false
          game.player.disableKeyboardNpcTalk = false
        when Types.Keys.DOWN, Types.Keys.S, Types.Keys.KEYPAD_2
  ...
  $(document).keydown (e) ->
    ...
    if game.started and not $("#chatbox").hasClass("active")
      #FIXME: when disconnecting "Cannot read property 'gridX' of null"
      pos =
        x: game.player.gridX
        y: game.player.gridY
  ...
  $("#chatinput").keydown (e) ->
    ...
    if key is 13
      if $chat.val() isnt ""
        game.say $chat.val()  if game.player
        $chat.val ""
        app.hideChat()
        $("#foreground").focus()
        return false
